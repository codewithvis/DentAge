...
export default function ResultsDashboardScreen({ navigation, route }) {
  ...
  const dentalAge = analysisData?.dental_age || 0;
  const chronoAge = 12.4; // This should come from patient data in a real app
  const delta = chronoAge ? parseFloat((dentalAge - chronoAge).toFixed(1)) : 0;
      ...
      const htmlContent = `
        <html>
          <body style="font-family: Arial; padding: 40px;">
            <h1>Dental Age Assessment Report</h1>
            <p><strong>Case ID:</strong> ${analysisData?.case_id || 'Unknown'}</p>
            <p><strong>Patient ID:</strong> ${analysisData?.patient_id || 'Unknown'}</p>
            <p><strong>Estimated Dental Age:</strong> ${dentalAge} years</p>
            <p><strong>Age Range:</strong> ${analysisData?.age_range || 'N/A'}</p>
            <p><strong>Development Stage:</strong> ${analysisData?.tooth_development_stage || 'N/A'}</p>
            <p><strong>AI Confidence:</strong> ${analysisData ? Math.round(analysisData.ai_confidence * 100) : 0}%</p>
            <p><strong>Maturity Score:</strong> ${analysisData?.maturity_score || 0}%</p>
            <h3>Analysis Summary</h3>
            <p>${analysisData?.analysis || 'AI analysis completed successfully.'}</p>
            <hr />
            <p><em>Generated by DentAge using Google Gemini AI</em></p>
          </body>
        </html>
      `;
    ...
    <SafeAreaView style={styles.safe}>
      ...
      >
        ...
        <View style={styles.maturityCard}>
          <Text style={styles.maturityLabel}>Dental Development Stage</Text>
          <View style={styles.maturityCircle}>
            <CircularProgress percentage={analysisData?.maturity_score || 0} value={analysisData?.tooth_development_stage || 'N/A'} />
          </View>
          <Text style={styles.maturitySub}>
            Maturity Score: {analysisData?.maturity_score || 0}% · {analysisData?.tooth_development_stage || 'Unknown Stage'}
          </Text>
        </View>
        ...
        <View style={styles.insightCard}>
          ...
          <Text style={styles.insightBody}>
            ...
            'AI-powered dental age assessment completed. The analysis provides detailed insights into tooth development stages and mineralization patterns observed in the OPG radiograph.'}
          </Text>
        </View>
        ...
        <View style={styles.infoCard}>
          ...
          <View style={styles.infoList}>
            {[
              { label: 'Development Stage', value: analysisData?.tooth_development_stage || 'N/A', highlight: true },
              { label: 'Radiograph Type', value: 'OPG Panoramic' },
              { label: 'Analysis Method', value: 'Gemini AI + Demirjian' },
              { label: 'Confidence', value: analysisData ? `${Math.round(analysisData.ai_confidence * 100)}%` : 'N/A' },
            ].map((item) => (
