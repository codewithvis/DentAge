...
serve(async (req: Request) => {
  ...
  try {
    ...
    try {
      ...
      if (typeof parsedAnalysis.age_range !== "string" || !/^\d+(\.\d+)?\s*-\s*\d+(\.\d+)?$/.test(parsedAnalysis.age_range)) {
        throw new Error(`Invalid age_range in AI response: ${parsedAnalysis.age_range}`);
      }
